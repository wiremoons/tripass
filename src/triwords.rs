...
pub static WORDS: &[&str] = &[
    ...
    "mod", "moe", "mog", "moi", "mol", "mom", "mon", "mit", "moo", "mop", "mor",
...
// Validation test for integrity maintenance of above arrays.
// Use: cargo test
...
mod tests {
    ...
    #[test]
    fn validate_sizes() {
        println!("INFO: Word dictionary count is: {}", WORDS.len());
        println!("INFO: Marks array count is: {}", MARKS.len());

        // Check that the word list is exactly the size you expect
        assert_eq!(
            WORDS.len(),
            1311,
            "The dictionary length must be exactly 1,311 words (actual is: {})",
            WORDS.len()
        );
        ...
        assert_eq!(
            MARKS.len(),
            10,
            "The marks array length must be exactly 10 symbols (actual is: {})",
            MARKS.len()
        );
    }

    #[test]
    fn validate_array_boundaries() {
        // Ensure the list starts and ends where we expect (alphabetical check)
        assert_eq!(
            WORDS[0], "aah",
            "The dictionary should start with 'aah' (actual is: {})",
            WORDS[0]
        );
        assert_eq!(
            WORDS[WORDS.len() - 1],
            "zzz",
            "The dictionary should end with 'zzz' (actual is: {})",
            WORDS[WORDS.len() - 1]
        );

        // Ensure marks boundaries are correct
        assert_eq!(
            MARKS[0], "#",
            "The first mark should be '#' (actual is: {})",
            MARKS[0]
        );
        assert_eq!(
            MARKS[MARKS.len() - 1],
            "<",
            "The last mark should be '<' (actual is: {})",
            MARKS[MARKS.len() - 1]
        );
    }

    #[test]
    fn validate_word_consistency() {
        // Since this is for a 3-letter password generator,
        // ensure no word was accidentally entered as 2 or 4 characters.
        for (i, word) in WORDS.iter().enumerate() {
            assert_eq!(
                word.len(),
                3,
                "Word at index {} ('{}') is not 3 letters long!",
                i,
                word
            );
        }
    ...
    fn check_for_duplicates() {
        // Ensure every word is unique to maximise password entropy.
        use std::collections::HashSet;
        let mut set = HashSet::new();
        for word in WORDS {
            assert!(set.insert(word), "Duplicate word found: {}", word);
        }
    }
}
